...
```{r}
set.seed(9)
training_data_1 = sample(1:nrow(no_churn_df), 0.9*nrow(no_churn_df))
training_data_2 = sample(1:nrow(yes_churn_df), 0.9*nrow(yes_churn_df))
...
of the AIC/BIC and confusion matrix efficiency that using only 1 variable resulted in a highly 
accurate model. 
...
Starting with a cutoff value of .17 as that is the ratio of yes's/no's in our data set.
```{r}
training_data$Predict = as.factor(ifelse(model$fitted.values > 0.17, "yes", "no"))
...
Looping through different options for the cutoff values to see which yields the higher confusion matrix efficiency
...
Cutoffs of .3 & .33 has the highest efficiency however we want to ensure we have a 
good balance of yes's and no's in our predictions to ensure we are minimizing
wasted incentives  along with maximizing accurate predictions
```{r}
#install.packages("yardstick")
#install.packages("ggplot2")
library(yardstick)
library(ggplot2)
training_data$Predict = as.factor(ifelse(model$fitted.values > 0.27, "yes", "no"))
truth <- data.frame(
...
test_data$Predicted = as.factor(ifelse(predicted > 0.27, "yes", "no"))

```

Viewing the predicted probalities to ensure our formatting is correct
...
Here we wanted to test different training percentages to examine the different 
efficiencies. 
...
#save(Churn_Prob, file = "Group_8.RData")
